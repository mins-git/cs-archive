...
* **개념**: 가장 높은 우선순위를 가진 프로세스에게 CPU를 할당.
    - 마찬가지로 starvation 문제가 생기기도함.

* **해결책 (Aging)**: Ready Queue에서 기다린 시간에 비례하여 프로세스의 우선순위를 점진적으로 높여주어 기아 현상을 방지.
    - 이렇게 해결방법이 존재하기는 하지만, 여전히 기아문제가 발생할 수 있기때문에 최근에는 RR스케줄링을 진행 하고있음.
...
* **개념**: 각 프로세스는 동일한 크기의 할당 시간(Time Quantum)을 가짐. 할당 시간이 지나면 선점당하고 Ready Queue의 맨 뒤로 이동.
   - 현대 컴퓨터가 많이 쓰는 방식으로 주방장이 타이머를 맞추고 1분동안 A 테이블 요리를 하다가 미완성이어도 B테이블로 요리를 1분하고 그다음 c를하는 방식.
    ...
    - 가장 좋은점은 응답시간이 가장 빠르다는것이다.
    - 누가 CPU를 오래쓸지 모르는 상황에서 굳이 예측할 필요없이 CPU선점권을 제공하기 때문에 적어도 N * Q (할당시간이 Q, N명이 존재할경우) 시간 이후에는 CPU를 점유할 수 있기때문에 좋음. 
    - CPU를 길게쓰는 프로그램은 길게 걸리게될것이고, CPU를 짧게쓰는 친구들은 짧게 진행됨.

* **특징**: Response Time이 매우 짧아짐. 할당 시간이 너무 크면 FCFS와 같아지고, 너무 작으면 Context Switch 오버헤드가 커짐.

    - 즉 Q가 LARGE => FCFS
    - Q가 SMALL -> CONTEXT SWITCH 오버헤드가 커지게 된다. (계속 CPU점유권을 계속 바꿔가기 때문에 오버헤드가 커지게 되는것이다.)
